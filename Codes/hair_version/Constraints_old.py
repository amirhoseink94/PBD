    ...
    for i in range(len(particles)):
        for j in range(i+1, len(particles)):
            if M[i, j] != 0:
                ...
                dp_1 = (-0.5 * (dist - M[i, j]) / dist) * ((particles[i].pred_pos - particles[j].pred_pos))
                dp_2 = (0.5 * (dist - M[i, j]) / dist) * ((particles[i].pred_pos - particles[j].pred_pos))
                k_1 = 1
                k_2 = 1
...
def constraint_projection_Q(particles, M, starts):
    (n, n) = M.shape
    visited_t = [0] * n
    inqu_t = [0] * n
